...
// The line to the server, and the callbacks waiting on a reply
let socket, connected = false, watching = false;
let nextReqId = 1, pending = {};
...
/**
 * Open the socket to the server; reconnects if the line drops
 */
function connect() {
...
function receive(o) {
    ...
    if ("reply" in o) {
        let callback = pending[o.reply];
        delete pending[o.reply];
        if (callback)
            callback(o.text);
    }
    // Info about the subjects
    else if (o.message == "client_info") {
...
function end_test(e) {
    ...
    // Stop the test; the reply comes once the current period has ended and the output file is written
...
/**
 * Asks the server for info about clients, now and whenever it changes
 */
function pend() {
    watching = true;
    send({ "type": "client_info" });
}
    ...
    setTimeout(function() {
        ...
        command("checkStatus", function(text) {
            ...
            if (sp.indexOf("practicing") == -1) {
                ...
                togglePractice.className = "endPractice";
            }
...
function command(query, callback) {
    ...
    ask({ "type": "request", "id": gid }, function(text) {
        let code = access_func.evalCode(+text);
        ask({ "type": "respond", "id": gid, "m": code, "quest": query }, function(text) {
            callback(text);
        });
    });
}
...
/**
 * Helper function for sending a message the server replies to
 */
